	...
	public:
		...
		struct tgt_elem{
			public:
				...
				float vtx_rf_dst[4];//distance fo vertex to the right fingertip;
		...
		void static set_tgts(std::vector<tgt_elem> tgts,std::vector<sqr*>& sbst,std::vector<sqr*>& wd_sbst){
			float vrtcs[8]; sgmnt sgmnt_grp[4],sgmnt_vt_ln;
			sgmnt_grp[0].set_data(grpr0.crds[0],grpr0.crds[1],grpr0.crds[4],grpr0.crds[5]);
			sgmnt_grp[1].set_data(grpr0.crds[2],grpr0.crds[3],grpr0.crds[6],grpr0.crds[7]);
			sgmnt_grp[2].set_data(grpr0.crds[0],grpr0.crds[1],grpr0.crds[2],grpr0.crds[3]);
			sgmnt_grp[3].set_data(grpr0.crds[4],grpr0.crds[5],grpr0.crds[6],grpr0.crds[7]);
			bool is_ib=0; unsigned short int usi_cnt=0; unsigned int ui_vec_cnt=0;
			for(unsigned int ulc=0;ulc<2;ulc++){
				std::vector<sqr*>* v_sqr_ptr;
				if(ulc==0){
					v_sqr_ptr=&sbst;
				}
				else{
				...
				for(unsigned int i=0;i<v_sqr_ptr->size();i++){//square;
					v_sqr_ptr->at(i)->get_vrtcs(vrtcs);
					for(unsigned int j=0;j<4;j++){//vertex;
						is_ib=0; usi_cnt=0;
						sgmnt_vt_ln.set_data(vrtcs[2*j],vrtcs[2*j+1],Max_x,vrtcs[2*j+1]);
						for(unsigned int k=0;k<4;k++){//segment;
							if(sgmnt::seg_crs_chck(sgmnt_grp[k],sgmnt_vt_ln)==cs){
								usi_cnt++;
							}
				
						}
						if(usi_cnt%2==1){//in the gripper binding box;
							if(is_ib==0){//tgt_elem is not recorded yet;
								tgt_elem tgt_elem_data;
								tgt_elem_data.sqr_elem=v_sqr_ptr->at(i);
								for(unsigned short int l=0;l<4;l++){
									tgt_elem_data.vtx_bx[l]=0;
									tgt_elem_data.vtx_lf_dst[l]=-1;
									tgt_elem_data.vtx_rf_dst[l]=-1;
						
								}
								tgts.push_back(tgt_elem_data);//tgt_elem is recorded;
								is_ib=1;
							}
							tgts.at(tgts.size()-1).vtx_bx[j]=1;
							if(sgmnt_grp[0].is_eq==1){
								tgts.at(tgts.size()-1).vtx_lf_dst[j]=abs(sgmnt_grp[0].x0-vrtcs[2*j]);
							}
							else{
								tgts.at(tgts.size()-1).vtx_lf_dst[j]=(abs(vrtcs[2*j+1]-sgmnt_grp[0].k*vrtcs[2*j]-sgmnt_grp[0].b))/sqrt(1+sgmnt_grp[0].k*sgmnt_grp[0].k);
							}
							if(sgmnt_grp[1].is_eq==1){
								tgts.at(tgts.size()-1).vtx_rf_dst[j]=abs(sgmnt_grp[1].x0-vrtcs[2*j]);
							}
							else{
								tgts.at(tgts.size()-1).vtx_rf_dst[j]=(abs(vrtcs[2*j+1]-sgmnt_grp[1].k*vrtcs[2*j]-sgmnt_grp[1].b))/sqrt(1+sgmnt_grp[1].k*sgmnt_grp[1].k);
							}
							if(0) cv::putText(data_img,std::to_string(ui_vec_cnt),cv::Point(vrtcs[2*j],vrtcs[2*j+1]),cv::FONT_HERSHEY_SIMPLEX,0.25,cv::Scalar(0,0,0));
							ui_vec_cnt++;
			...
			if(0){
				unsigned int ui_tgts_cnt=0;
				for(unsigned int i=0;i<tgts.size();i++){
					for(unsigned int j=0;j<4;j++){
						if(tgts.at(i).vtx_bx[j]==1){
							std::cout<<"\nui_tgts_cnt="<<ui_tgts_cnt<<" tgts.at(i).vtx_lf_dst[j]="<<tgts.at(i).vtx_lf_dst[j]<<" tgts.at(i).vtx_rf_dst[j]="<<tgts.at(i).vtx_rf_dst[j];
							ui_tgts_cnt++;
				...
				ui_tgts_cnt=0;
				for(unsigned int i=0;i<tgts.size();i++){
					tgts.at(i).sqr_elem->get_vrtcs(vrtcs);
					for(unsigned int j=0;j<4;j++){
						if(tgts.at(i).vtx_bx[j]==1){
	 						cv::putText(data_img,std::to_string(ui_tgts_cnt),cv::Point(vrtcs[2*j],vrtcs[2*j+1]),cv::FONT_HERSHEY_SIMPLEX,0.25,cv::Scalar(255,255,255));
							ui_tgts_cnt++;
						...
						if(sqrt((p0x-p1x)*(p0x-p1x)+(p0y-p1y)*(p0y-p1y))>grpr_t0::fms){
							...
							for(unsigned short int curr_fr=0;curr_fr<(frcs+2);curr_fr++){//fractions;
								...
								if(sqr::ft_cnfg_chck(wd_sbst)==0){//no intersections with fingertips;
									std::cout<<"\nni!";
									std::vector<tgt_elem> tgts;
									sqr::set_tgts(tgts,sbst,wd_sbst);
